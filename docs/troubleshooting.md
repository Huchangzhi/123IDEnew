...
- [macOS](#macos)
  ...
  - ["IDE123.app" is damaged and can’t be opened. You should move it to the Bin](#macos-quarantine)
...
You have likely encountered [a bug in Chromium and Electron](microsoft/vscode#190437) when compiling Mesa shaders, which has affected all Visual Studio Code and IDE123 versions for Linux distributions since 1.82.  The current workaround (see microsoft/vscode#190437) is to delete the GPU cache as follows:

```bash
rm -rf ~/.config/IDE123/GPUCache
```
...
Credits: [Gerson](https://gitlab.com/paulcarroty/IDE123-deb-rpm-repo/-/issues/91)

#### <a id="linux-flatpak-most-common-issues"></a>*Flatpak most common issues*

- blurry screen with HiDPI on wayland run:
  ```bash
  flatpak override --user --nosocket=wayland com.IDE123.codium
  ```
...
- SDKs
  see [this](https://github.com/flathub/com.IDE123.codium?tab=readme-ov-file#sdks)

- If you have any other problems with the flatpak package try to look on the [FAQ](https://github.com/flathub/com.IDE123.codium?tab=readme-ov-file#faq) maybe the solution is already there or open an [issue](https://github.com/flathub/com.IDE123.codium/issues).

##### <a id="linux-remote-ssh"></a>*Remote SSH doesn't work*

Use the IDE123's compatible extension [Open Remote - SSH](https://open-vsx.org/extension/jeanp413/open-remote-ssh).
...
#### <a id="macos-quarantine"></a>*"IDE123.app" is damaged and can’t be opened. You should move it to the Bin.*

The following command will remove the quarantine attribute.

```
xattr -r -d com.apple.quarantine /Applications/IDE123.app
```

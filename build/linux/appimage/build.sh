...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  # add update's url
  sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|IDE123|IDE123|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun

  # remove check so build in docker can succeed
  sed -i 's/grep docker/# grep docker/' pkg2appimage.AppDir/usr/share/pkg2appimage/functions.sh

  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|IDE123-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|codium-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|IDE123-insiders|g' recipe.yml
  else
    sed -i 's|@@NAME@@|IDE123|g' recipe.yml
    sed -i 's|@@APPNAME@@|codium|g' recipe.yml
    sed -i 's|@@ICON@@|IDE123|g' recipe.yml
  fi

  # workaround that enforces x86 ARCH for pkg2appimage having /__w/IDE123/IDE123/build/linux/appimage/IDE123/IDE123.AppDir/usr/share/codium/resources/app/node_modules/rc/index.js is of architecture armhf
  export ARCH=x86_64
